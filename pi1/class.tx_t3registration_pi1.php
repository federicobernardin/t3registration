...
class tx_t3registration_pi1 extends tslib_pibase {
    ...
    protected function getForm() {
        ...
        if (($preview && !$this->conf['enablePreview']) || ($this->piVars['sendConfirmation'] == 1 && isset($this->piVars['confirmPreview']) && $preview) || (isset($this->userLogged) && $this->userLogged && isset($this->piVars['confirmPreview']) && $preview)) {
    ...
    public function getAndReplaceSubpartPreview($field, $content, $contentArray) {
        if ($field['hideInChangeProfile'] == 1 && $this->userLogged) {
            ...
            $contentArray['###' . strtoupper($field['name']) . '_LABEL###'] = (($field['hideInChangeProfile'] == 1 && $this->userLogged) || (isset($this->piVars[$field['name']]) && !is_array($this->piVars[$field['name']]) && strlen($this->piVars[$field['name']]) == 0) && (isset($this->conf['form.']['hideInPreviewIfEmpty']) && $this->conf['form.']['hideInPreviewIfEmpty'] == 1)) ? '' : (($this->pi_getLL($field['name'] . 'Label')) ? $this->cObj->stdWrap($this->pi_getLL($field['name'] . 'Label'), $this->conf['form.']['standardPreviewLabelWrap.']) : ((isset($field['label'])) ? $this->cObj->stdWrap($this->languageObj->sL($field['label'], true), $this->conf['form.']['standardPreviewLabelWrap.']) : ''));
